...
export async function POST(request: NextRequest) {
  ...
  try {
    const {
      ...
      chunkIndex,
      includeImages = true,
    }: ChunkProcessRequest = await request.json();
    ...
    logger.info(
      "PIPELINE_CHUNK",
      `Chunk ${chunkIndex + 1}: Generating batch requests...`,
      { chunkIndex }
    );
    ...
    const batchRequests = await geminiService.createBatchRequests(
      ...
      (processed, total) => {
        ...
        if (processed % 10 === 0) {
          logger.debug(
            "PIPELINE_CHUNK",
            `Chunk ${chunkIndex + 1}: ${processed}/${total} rugs processed`,
            {
              chunkIndex,
              processed,
              total,
    ...
    const skuMapping = rugs.map((rug, idx) => ({
      index: idx,
      sku: rug.sku || `idx-${idx}`,
      key: rug.sku ? `rug-${rug.sku}` : `rug-idx-${idx}`,
    }));

    logger.info(
      "PIPELINE_CHUNK",
      `Chunk ${chunkIndex + 1}: Generated ${
    ...
    logger.info(
      "PIPELINE_CHUNK",
      `Chunk ${chunkIndex + 1}: Submitting to Gemini...`,
      { chunkIndex }
    );
    ...
    const submitResponse = await fetch(
      ...
      }/api/submit-batch`,
      {
        ...
        body: JSON.stringify({
          jsonlContent,
          displayName: `Rug Batch Chunk ${chunkIndex + 1} - ${
    ...
    logger.info(
      "PIPELINE_CHUNK",
      `Chunk ${chunkIndex + 1}: Successfully submitted`,
      {
        chunkIndex,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        requestCount: batchRequests.length,
        skuMapping, // Include SKU mapping for image extraction
        duration,
      },
    });
  } catch (error) {
